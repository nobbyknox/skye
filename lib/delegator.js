...
const delegatorLoopInterval = 5000;
...
function start() {
    ...
    setInterval(() => {
        ...
        if (!taskAllocation[constants.TASKS.PROCESS_IMAGES]) {
            ...
            if (taskAllocation[constants.TASKS.PROCESS_IMAGES]) {
                logger.debug('Task "%s" allocated to peer "%s"', constants.TASKS.PROCESS_IMAGES, taskAllocation[constants.TASKS.PROCESS_IMAGES]);
            }
        }
        ...
        if (!taskAllocation[constants.TASKS.SEND_NOTIFICATIONS]) {
            ...
            if (taskAllocation[constants.TASKS.SEND_NOTIFICATIONS]) {
                logger.debug('Task "%s" allocated to peer "%s"', constants.TASKS.SEND_NOTIFICATIONS, taskAllocation[constants.TASKS.SEND_NOTIFICATIONS]);
            }
        }
